...
import scitzen.sast.{Block, Directive, Fenced, Section}
...
case class SastContext[+T](
  ...
  def addConversionBlock(block: Fenced): SastContext[T] = copy(convertBlocks = block :: convertBlocks)
