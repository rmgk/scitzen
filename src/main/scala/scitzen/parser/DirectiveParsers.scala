...
import scitzen.sast.{Attribute, Attributes, DCommand, Directive, InlineText, Prov}
...
object DirectiveParsers {
  ...
  val full: Scip[Directive] = Scip {
    ...
    Directive(DCommand.parseMacroCommand(name.getOrElse("")), Attributes(attributes))(Prov(start, scx.index))
  }
  ...
  val comment: Scip[Directive] = Scip {
    ...
    Directive(Comment, Attribute("", text).toAttributes)(Prov(start, scx.index))
  }
